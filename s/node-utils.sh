...
_ent-bundle-deploy() {
  ...
  local TENANT_CODES OVERWRITE_TENANTS FORCE_OVERWRITE_TENANTS OVERWRITE_TENANTS_FLAGS BUNDLE_CR
  ...
  BUNDLE_CR="$(ecr.docker.generate-cr "" "$TENANT_CODES" "$OVERWRITE_TENANTS" "$FORCE_OVERWRITE_TENANTS")"

  if [ -n "$BUNDLE_CR" ]; then
    echo "$BUNDLE_CR" | _kubectl apply -f -
  fi
